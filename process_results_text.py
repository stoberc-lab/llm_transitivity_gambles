...
def lookup_choices(test_name, choices, choice_num=None):
    ...
    if choice_num:
        chosen_code = choice_codes[choice_num - 1]
        return choice_codes, chosen_code
    else:
        return choice_codes
                ...
                for out_file in out_files:
                    ...
                    with open(f"{prompt_num_dir}/{out_file}", "r") as cur_file:
                        ...
                        for line in file_lines:
                            line = line.strip()
                            if is_response_line:
                                new_data["response_obj"] = ast.literal_eval(line)
                                try:
                                    new_data["choice_num"] = int(
                                        new_data["response_obj"]["response"][-1]
                                    )
                                except ValueError as e:
                                    new_data["choice_num"] = -1
                                is_response_line = False
                                ...
                                pass
                    if "temperature" not in new_data:
                        new_data["temperature"] = 0

                    if new_data["choice_num"] not in [1, 2]:
                        choice_codes = lookup_choices(
                            new_data["test_name"], new_data["choices"]
                        )
                        handle_error(
                            model,
                            f"{prompt_num_dir}/{out_file}",
                            new_data["prompt_name"],
                            new_data["text_format"],
                            new_data["question_num"],
                            new_data["temperature"],
                            "_".join(choice_codes),
                            new_data["choice_num"],
                            new_data["response_obj"]["response"],
                        )
                        continue

                    new_data["choice"] = new_data["choices"][new_data["choice_num"] - 1]
                    choice_codes, chosen_code = lookup_choices(
                        new_data["test_name"],
                        new_data["choices"],
                        new_data["choice_num"],
                    )
                    ...
                    new_data["first_code_choice_selected"] = int(
                        choice_codes[0] == chosen_code
                    )

                    del new_data["choice"]
                    del new_data["choices"]
                    del new_data["choice_num"]
                    del new_data["response_obj"]
...
if len(errors) > 0:
    errors_file_path = f"./analysis/{benchmark}/{config_name}/errors.csv"
    Path(os.path.dirname(os.path.abspath(errors_file_path))).mkdir(
        parents=True, exist_ok=True
    )

    print(
        f'Encountered {len(errors)} errors in parsing choice outputs. Errors will be written to: "{errors_file_path}"'
    )
    with open(errors_file_path, "w", newline="") as errors_file:
        dict_writer = csv.DictWriter(errors_file, list(errors[0].keys()))
...
for prompt_name, prompt_name_data in data_by_prompt_name.items():
    ...
    for data in prompt_name_data:
        ...
        aggregated_data[agg_key][data["question_choice_codes"]] += data[
            "first_code_choice_selected"
        ]
        aggregated_data[agg_key][f'{data["question_choice_codes"]}_COUNT'] += 1
