...
def lookup_choices(test_name, choices, chosen_choice, choice_num):
                ...
                for out_file in out_files:
                    ...
                    with open(f"{prompt_num_dir}/{out_file}", "r") as cur_file:
                        ...
                        for line in file_lines:
                            ...
                            # print(line)
                            if is_response_line:
                                response_json = json.loads(line.replace("'", '"'))
                    ...
                    new_data["order"] = new_data["test_name"][
                        new_data["test_name"].rindex("order") :
                    ]
                    new_data["test_name"] = new_data["test_name"][
                        : new_data["test_name"].rindex("_order")
                    ]
...
for prompt_name, prompt_name_data in data_by_prompt_name.items():
    ...
    for data in prompt_name_data:
        ...
        agg_key = f'{data["model_name"]}|{data["test_name"]}|{prompt_name}|{data["temperature"]}|{data["order"]}|{data["text_format"]}'
